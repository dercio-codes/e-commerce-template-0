...
export default function ProductContainer(props) {
    ...
    try {
      ...
      if (productData.includes("")) {
        ...
        localStorage.setItem(
          "authUser",
          JSON.stringify({ ...user, cart: [...localCart] })
        );
  ...
  return (
    ...
    >
      ...
      >
        <Box
          sx={{ display: "flex", alignItems: "center", width: { xs: "50%" } }}
        >
          <TextField
            value={sortBy}
            select
          ...
          >
            {SortBy.map((item) => (
              <MenuItem onClick={() => setSortBy(item)} key={item} value={item}>
                {item}
              </MenuItem>
            ))}
          </TextField>
            ...
            return (
              <Grid key={item + index} item xs={6} md={4} lg={2.4}>
                <ProductItem
                  ...
                  product={item}
                  color={
                    index % 2 === 0 ? "rgba(255,0,0,.8)" : "rgba(0,200,0,.8)"
                  }
                  special={index % 2 === 0 ? "Hot In" : "Sale"}
                  image={item}
                  Title={"Item Name"}
                  Price={"R500"}
                />
